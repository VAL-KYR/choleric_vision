...
namespace VolumetricFogAndMist
{
	...
	public partial class VolumetricFog : MonoBehaviour
	{
		...
		[SerializeField]
		int
			_sunShadowsResolution = 3;	// default = 2^(9+3) = 4096 texture size
		...
		[SerializeField]
		float
			_sunShadowsRefreshInterval = 0;	// 0 = no update unless Sun changes position
		...
		public float turbulenceStrength {
			...
			set {
				if (value != _turbulenceStrength) {
					...
					if (_turbulenceStrength<=0f) UpdateTexture();	// reset texture to normal
					UpdateMaterialProperties ();
					isDirty = true;
				}
			}
		...
		void Init ()
		{
			...
			#if UNITY_EDITOR
			if (PlayerSettings.virtualRealitySupported) _improveTransparency = false;
			#endif
		}
		...
		internal void DoOnRenderImage (RenderTexture source, RenderTexture destination)
		{
			...
			if (mainCamera.orthographic) fogMat.SetVector("_ClipDir", mainCamera.transform.forward);
			...
			// Apply chaos
			if (Application.isPlaying && _turbulenceStrength>0f) ApplyChaos();
			...
			if (_downsampling > 1f) {
				...
				if (!_edgeImprove || UnityEngine.VR.VRSettings.enabled || SystemInfo.supportedRenderTargetCount < 2) {
		...
		void UpdateTexture ()
		{
			...
			if (adjustedChaosTexture != null) DestroyImmediate(adjustedChaosTexture);
