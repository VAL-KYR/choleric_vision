...
namespace VolumetricFogAndMist
{
	...
	public class VolumetricFogInspector : Editor
	{
		...
		const string SCATTERING_ON = "Sun Shafts (ON)";
		const string SCATTERING_OFF = "Sun Shafts";
		...
		const string VFM_BUILD_HINT = "VFMBuildHint71";
		VolumetricFog _fog;
		...
		void OnDestroy ()
		{
			...
			EditorPrefs.SetBool ("expandFogStyleSection", expandFogColorsSection);
		...
		public override void OnInspectorGUI ()
		{
			...
			if (EditorPrefs.GetInt (VFM_BUILD_HINT) == 0) {
				...
				EditorUtility.DisplayDialog ("Volumetric Fog & Mist v7.1", "Before building your game, please check the Build Options section in Volumetric Fog inspector for new shader capabilities and make sure you disable unused features to optimize build size and compilation time.\n\nOtherwise when you build the game it will take a long time or even get stuck during shader compilation.", "Ok");
			}
			...
			bool transparencySupport = true;
			#if UNITY_EDITOR
			if (PlayerSettings.virtualRealitySupported) transparencySupport = false;
			#endif
			...
			if (expandFogGeometrySection) {
				...
				EditorGUILayout.BeginHorizontal ();
				GUILayout.Label (new GUIContent ("Max. Dist. FallOff", "Blends far range with background."), GUILayout.Width (120));
				_fog.maxFogLengthFallOff = EditorGUILayout.Slider (_fog.maxFogLengthFallOff, 0f, 1f);
				EditorGUILayout.EndHorizontal ();
				...
				EditorGUILayout.BeginHorizontal ();
				GUILayout.Label (new GUIContent ("Use XY Plane", "Switches between normal mode to XY mode."), GUILayout.Width (120));
				if (IsFeatureEnabled (VolumetricFog.SKW_FOG_USE_XY_PLANE)) {
					_fog.useXYPlane = EditorGUILayout.Toggle (_fog.useXYPlane);
				} else {
				...
				EditorGUILayout.BeginHorizontal ();
				if (_fog.useXYPlane) {
					GUILayout.Label (new GUIContent ("Base Z", "Starting Z of the fog in meters."), GUILayout.Width (120));
				} else {
				...
				if (_fog.useXYPlane) GUI.enabled = false;
				EditorGUILayout.BeginHorizontal ();
			...
			if (expandFogAnimationSection) {
				...
				EditorGUILayout.BeginHorizontal ();
				GUILayout.Label (new GUIContent ("Turbulence", "Turbulence strength. Set to zero to deactivate. Turbulence adds a render pass to compute fog animation."), GUILayout.Width (120));
				_fog.turbulenceStrength = EditorGUILayout.Slider (_fog.turbulenceStrength, 0, 10f);
				EditorGUILayout.EndHorizontal ();
			}
			...
			if (IsFeatureEnabled (VolumetricFog.SKW_SUN_SHADOWS)) {
				expandSunShadowsSection = EditorGUILayout.Foldout (expandSunShadowsSection, _fog.sunShadows ? SUN_SHADOWS_ON : SUN_SHADOWS_OFF, sectionHeaderStyle);
				
				if (expandSunShadowsSection) {
					EditorGUILayout.EndHorizontal ();
					
					EditorGUILayout.BeginHorizontal ();
					GUILayout.Label (new GUIContent ("Enable", "Enables Sun shadow casting over fog."), GUILayout.Width (120));
					_fog.sunShadows = EditorGUILayout.Toggle (_fog.sunShadows);
					EditorGUILayout.EndHorizontal ();
					
					if (_fog.sunShadows && _fog.sun == null) {
						EditorGUILayout.BeginHorizontal ();
						GUILayout.Box ("Warning: Sun Shadows requires a Sun reference. Assign a game object (representing Sun or a light) in General Settings section.");
						EditorGUILayout.EndHorizontal ();
					}
					
					EditorGUILayout.BeginHorizontal ();
					GUILayout.Label (new GUIContent ("Strength", "Shadow strength. "), GUILayout.Width (120));
					_fog.sunShadowsStrength = EditorGUILayout.Slider (_fog.sunShadowsStrength, 0, 1);
					EditorGUILayout.EndHorizontal ();
					
					EditorGUILayout.BeginHorizontal ();
					GUILayout.Label (new GUIContent ("Resolution", "Resolution of the internal shadow texture. Sizes: 0=512, 1=1024, 2=2048, 3=4096, 4=8192."), GUILayout.Width (120));
					_fog.sunShadowsResolution = EditorGUILayout.IntSlider (_fog.sunShadowsResolution, 0, 4);
					EditorGUILayout.EndHorizontal ();

					EditorGUILayout.BeginHorizontal ();
					GUILayout.Label (new GUIContent ("Max Distance", "Shadow visibility maximum distance from camera. It cannot exceed the fog max distance value in Fog Geometry section."), GUILayout.Width (120));
					_fog.sunShadowsMaxDistance = EditorGUILayout.Slider (_fog.sunShadowsMaxDistance, 50f, 2000f);
					EditorGUILayout.EndHorizontal ();
					
					EditorGUILayout.BeginHorizontal ();
					GUILayout.Label (new GUIContent ("Force Update Interval", "Updates shadow texture after specified time interval (in seconds). Set this value to 0 to ignore shadow updating. Note that it will update whenever the Sun rotates."), GUILayout.Width (120));
					_fog.sunShadowsRefreshInterval = EditorGUILayout.FloatField (_fog.sunShadowsRefreshInterval, GUILayout.Width (40));
					EditorGUILayout.EndHorizontal ();

					EditorGUILayout.BeginHorizontal ();
					GUILayout.Label (new GUIContent ("Jittering", "Helps reducing shadow banding artifacts."), GUILayout.Width (120));
					_fog.sunShadowsJitterStrength = EditorGUILayout.Slider (_fog.sunShadowsJitterStrength, 0, 0.2f);
				}
			} else {
				EditorGUILayout.Foldout (false, SCATTERING_OFF, sectionHeaderStyle);
				DrawFeatureIsDisabled ();
			...
			if (IsFeatureEnabled (VolumetricFog.SKW_FOG_BLUR)) {
				expandFogBlurSection = EditorGUILayout.Foldout (expandFogBlurSection, _fog.fogBlur ? FOG_BLUR_ON : FOG_BLUR_OFF, sectionHeaderStyle);
				
				if (expandFogBlurSection) {
					EditorGUILayout.EndHorizontal ();
					
					EditorGUILayout.BeginHorizontal ();
					GUILayout.Label (new GUIContent ("Enable", "Applies a depth blur effect to the fog to simulate extra light scattering."), GUILayout.Width (120));
					_fog.fogBlur = EditorGUILayout.Toggle (_fog.fogBlur);
					EditorGUILayout.EndHorizontal ();
					
					EditorGUILayout.BeginHorizontal ();
					GUILayout.Label (new GUIContent ("Depth Threshold", "Starting distance for depth blur."), GUILayout.Width (120));
					_fog.fogBlurDepth = EditorGUILayout.Slider (_fog.fogBlurDepth, 0, 1f);
				}
			} else {
			...
			if (expandOptionalPointLightSection) {
				...
				EditorGUILayout.BeginHorizontal ();
				GUILayout.Label (new GUIContent ("  Check Interval", "Time interval between checks of nearest point lights. If you have up to 6 point lights in the scene, this parameter can be ignored and should be set to the maximum interval. If you have more than 6 point lights, this parameter determines the interval between distances checks to choose which lights (up to 6) illuminate the fog."), GUILayout.Width (120));
				_fog.pointLightTrackingCheckInterval = EditorGUILayout.Slider (_fog.pointLightTrackingCheckInterval, 0, 5);
				EditorGUILayout.EndHorizontal ();
			...
			if (expandOptimizationSettingsSection) {
				...
				_fog.downsampling = EditorGUILayout.IntSlider (_fog.downsampling, 1, 8);
				EditorGUILayout.EndHorizontal ();
